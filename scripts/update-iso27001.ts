...
async function updateISO27001() {
  try {
    console.log('Updating existing ISO 27001 framework...');
    ...
    const frameworkId = existingFramework[0].id;
    console.log(`Found framework ID: ${frameworkId}`);

    // Update the framework description
    await db
      .update(frameworks)
      .set({
    ...
    console.log('Updated framework description and version');
    ...
    // Delete existing controls (will cascade delete templates)
    ...
    // Create 39 new controls
    console.log('Creating 39 new controls...');
...
*This template is part of the ISO 27001:2022 compliance framework implementation.*`
    ...
    console.error('❌ Error updating ISO 27001 framework:', error);
  } finally {
    await pool.end();
  }
}
